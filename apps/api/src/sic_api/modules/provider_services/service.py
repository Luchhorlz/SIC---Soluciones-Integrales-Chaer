...
class ProviderOfferService:
    ...
    def __init__(self, repository: ProviderServiceRepository, catalog: CatalogService, addresses: AddressRepository) -> None:
    ...
    def _view(self, configuration: ProviderServiceConfiguration, profile: ProviderProfileView) -> ProviderServiceView:
    ...
    async def list(self, profile: ProviderProfileView) -> list[ProviderServiceView]:
        return [self._view(item, profile) for item in await self.repository.list(profile.id)]
    ...
    async def create(self, user_id: UUID, profile: ProviderProfileView, payload: ProviderServiceCreate) -> ProviderServiceView:
        ...
        return self._view(await self.repository.create(profile.id, payload, await self._center(user_id, payload)), profile)
    ...
    async def update(self, user_id: UUID, profile: ProviderProfileView, item_id: UUID, payload: ProviderServiceUpdate) -> ProviderServiceView:
        ...
        return self._view(await self.repository.update(profile.id, item_id, effective, await self._center(user_id, effective)), profile)

    async def set_paused(self, profile: ProviderProfileView, item_id: UUID, paused: bool) -> ProviderServiceView:
        return self._view(await self.repository.set_paused(profile.id, item_id, paused), profile)
