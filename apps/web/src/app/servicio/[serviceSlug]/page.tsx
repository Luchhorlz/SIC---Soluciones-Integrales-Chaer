...
export default async function ServicePage({ params }: Props) {
  ...
  const match = findService(await getPublicCatalog(), serviceSlug);
  if (!match) notFound();
  const providers = await searchProviders({ service_slug: serviceSlug, mode: "ALL", limit: 12 });
  ...
  return (
    <main><PublicHeader />
      ...
      <section className="service-provider-results"><div className="section-heading"><div><p className="eyebrow">OPCIONES PÚBLICAS</p><h2>Prestadores disponibles</h2></div><span>{providers.count} resultados</span></div>{providers.demoData && <div className="demo-catalog-notice"><b>3 profesionales demo para este servicio</b><span>Son perfiles ficticios creados para probar la navegación completa de SIC.</span></div>}{providers.results.length ? <div className="home-provider-list">{providers.results.map((result) => <ProviderResultCard result={result} key={result.provider_slug} />)}</div> : <div className="public-empty-state"><span>◎</span><h3>No hay prestadores visibles para este servicio</h3><p>Podés usar el buscador superior y autorizar tu ubicación para consultar cobertura presencial.</p></div>}</section>
